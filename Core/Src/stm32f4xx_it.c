...
void USART1_IRQHandler(void)
{
  /* USER CODE BEGIN USART1_IRQn 0 */
    __HAL_UNLOCK(&huart1); /* è§£é”?ï¼Œé?¿å…?æ­»é”? */
...
//			HAL_GPIO_WritePin(SERIAL_SERVO_TX_EN_GPIO_Port, SERIAL_SERVO_TX_EN_Pin, GPIO_PIN_SET);
    ...
    if(__HAL_UART_GET_FLAG(&huart5, UART_FLAG_TXE) != RESET) {
        ...
        if(serial_servo_controller.tx_byte_index < serial_servo_controller.tx_frame.elements.length + 3) {  /* åˆ¤æ–­æ•°æ?®æ˜¯å?¦å?‘å?‘é€?å®Œæˆ? */
            huart5.Instance->DR = ((uint8_t*)(&serial_servo_controller.tx_frame))[serial_servo_controller.tx_byte_index++]; /* ç»§ç»­å?‘é¿?ä¸‹ä¸¿ä¸ªå­—èŠ? */
        } else {
